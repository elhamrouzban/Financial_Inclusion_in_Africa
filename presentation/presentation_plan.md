...
### Purpose
Explain how raw survey data becomes model-ready data.

### Recommended visual
A pipeline diagram rather than code:
...
### Current verified / planned items
- Target: convert `Yes/No` to binary representation when required by the model.
- `uniqueid` should not be treated as a meaningful predictive feature.
- Categorical features require encoding.
- Numerical features may require scaling depending on the model.
- Use a clean reproducible preprocessing pipeline, ideally with `ColumnTransformer`.

### Feature engineering ideas to evaluate
Only include after testing:
- age groups
- household-size groups
- possible interactions involving country / education / employment
- other domain-informed features

### Important rule
Do not present untested feature engineering as completed work.
...
### Purpose
Explain what “good performance” means before showing model results.

### Evaluation challenge
Because only **14.08%** of respondents are in the `Yes` class, accuracy alone can hide poor minority-class performance.
...
- gives both classes equal weight;
- combines precision and recall;
- is appropriate for comparing models on this imbalanced classification problem.
...
### Baseline definition
The first baseline should be a **DummyClassifier / majority-class predictor**.

### Verified baseline result
Dummy majority model:
- Accuracy: **85.93%**
- Error Rate: **14.07%**
- Macro F1: **0.46**
- `Yes` recall: **0.00**
...
### Main message
A model can achieve ~86% accuracy while completely failing to identify bank-account owners.

### Recommended visual
**Confusion matrix + one metric callout**

Callout:
**85.93% accuracy, but 0% recall for the Yes class**

This is stronger than showing only a metric table.

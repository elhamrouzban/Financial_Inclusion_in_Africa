...
6. [Turtle Rescue Forecast Challenge](https://zindi.africa/competitions/turtle-rescue-forecast-challenge) (Medium, has a starter notebook, older dataset)
...
2. A Jupyter notebook following PEP 8, designed for a data science and technical audience.
3. A Python script to generate and run your model from the terminal (nice to have). One script takes a `.csv` as an argument and saves the model locally. Another takes `test.csv` and the model as arguments and outputs accuracy and predictions as a `.csv`. You can build on this template repository for that.
4. One slide about a potential data product: how could the predictions be used?
...
Sample notebooks from past Zindi challenges:

- [Tanzania Tourism (data preparation)](https://github.com/daniel-obare/-Kenya-Hack---Tanzania-Tourism-Prediction-Challenge-by-Nairobi-Women-In-Machine-Learning-Data-Scienc/blob/main/Train_set_data_preparation.ipynb)
- [Financial Inclusion in Africa](https://github.com/Hilel93/Financial-Inclusion-in-Africa/blob/master/Financial.ipynb)
- [Turtle Rescue Forecast](https://github.com/souhagaa/Sea-Turtle-Rescue-Forecast-Challenge/blob/master/hack_for_earth.ipynb)
